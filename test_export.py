...
from nbconvert_styles import KULeuvenExporter
...
def test_template_export():
    """
    Run the generation of the document from the template's exporter.
    """
    for test_file_name in ["KULeuven slides test.ipynb","Sedes KULeuven slides test.ipynb","KULeuven 1425 test.ipynb"]:
        test_file = get_test_filepath(test_file_name)
        stream, info = KULeuvenExporter().from_filename(test_file)
        try:
